...
class WaveletDatagenerator(tf.keras.utils.Sequence):
  ...
  def __data_generation(self, batch_indices):
    ...
    X = np.empty((self.batch_size, 3, self.data_dict[0].shape[1]-1, self.data_dict[0].shape[1] ))
    ...
    for i,j in enumerate(batch_indices):
      if self.augment_data and (WaveletDatagenerator.number_of_generated_samples % 4) == 0:
        ...
        for k in range(3):
          coefficients, frequencies = pywt.cwt(self.data_dict[j][k,:], scales, 'morl')
          coefs_temp.append(coefficients)
        X[i,] = np.array(coefs_temp)/255.
...
def plot_coefficients(coefficients_array, attributes, G, subject_index = 0, specific_subject = True):
  """
...
def predict(WL_saved_model, test_data):
  ...
  Dict = {i:j for i,j in enumerate(['Relax', 'PhysicalStress', 'EmotionalStress', 'CognitiveStress']) }
...
def plot_data_from_DATADICT(DATADICT, NUM_2_LABELS, wavelet = 'morl', index = 63):
  NUM_2_LABELS = {i:j for i,j in enumerate(['SpO2', 'HeartRate', 'AccX', 'AccY', 'AccZ', 'Temp', 'EDA'])}
  ...
  fig, axs = plt.subplots(1, sample.shape[1] , figsize=(15, 6), facecolor='w', edgecolor='k')
  fig.subplots_adjust(hspace = .5, wspace=.001)
  axs = axs.ravel()
  for i in range(7):
