...
def SortSPO2HR(base_dir, Subjects):
  ...
  SPO2HR_attributes_dict = {'Relax': [], 'CognitiveStress': [], 'PhysicalStress': [], 'EmotionalStress': []}
  ...
  for i in Subjects:
    ...
    assert(len(relax_spo2) + len(physical_spo2) + len(emotional_spo2) + len(cognitive_spo2) == len(labels))
    ...
    for i in labels_dict.keys(): # (Relax, PhysicalStress, CognitiveStress, EmotionalStress)
...
def SortAccTempEDA(base_dir, Subjects):
  ...
  AccTempEDA_attributes_dict = {'Relax':[], 'PhysicalStress': [], 'CognitiveStress': [], 'EmotionalStress': []}
  ...
  for i in Subjects:
    ...
    emotional_AccY, emotional_AccX, emotional_AccZ, emotional_Temp, emotional_EDA = [], [], [], [], []
    cognitive_AccY, cognitive_AccX, cognitive_AccZ, cognitive_Temp, cognitive_EDA = [], [], [], [], []

    temp_dict1 = {'AccX': AccX, 'AccY': AccY, 'AccZ': AccZ, 'Temp': Temp, 'EDA': EDA}
    temp_dict2 = {
        'RelaxAccY': Relax_AccY, 'RelaxAccX': Relax_AccX, 'RelaxAccZ': Relax_AccZ, 'RelaxTemp': Relax_Temp, 'RelaxEDA': Relax_EDA,
        'PhysicalStressAccY': physical_AccY, 'PhysicalStressAccX': physical_AccX, 'PhysicalStressAccZ':physical_AccZ, 'PhysicalStressTemp':physical_Temp, 'PhysicalStressEDA':physical_EDA,
        'EmotionalStressAccY': emotional_AccY, 'EmotionalStressAccX': emotional_AccX, 'EmotionalStressAccZ':emotional_AccZ, 'EmotionalStressTemp':emotional_Temp, 'EmotionalStressEDA':emotional_EDA,
        'CognitiveStressAccY': cognitive_AccY, 'CognitiveStressAccX': cognitive_AccX, 'CognitiveStressAccZ': cognitive_AccZ, 'CognitiveStressTemp': cognitive_Temp, 'CognitiveStressEDA': cognitive_EDA
    }
    ...
    assert(len(temp_dict2['PhysicalStressAccX']) == len(temp_dict2['PhysicalStressAccZ']))
    assert(len(temp_dict2['CognitiveStressAccX']) == len(temp_dict2['CognitiveStressEDA']))
    assert(len(temp_dict2['EmotionalStressAccX']) == len(temp_dict2['EmotionalStressTemp']))
...
def plot_varying_recording_time(SPO2HR_attributes_dict, AccTempEDA_attributes_dict ):
  ...
  plt.subplot(2,2,1)
  plt.scatter(rx, r)
  plt.title('Varying total recording time of Relax for each subject in SpO2HR.csv file')
  plt.subplot(2,2,2)
  plt.scatter(px, p)
  plt.title('Varying total recording time of PhysicalStress for each subject SpO2HR.csv file')
  plt.subplot(2,2,3)
  plt.scatter(cx, c)
  plt.title('Varying total recording time of CognitiveStress for each subject SpO2HR.csv file')
  plt.subplot(2,2,4)
  plt.scatter(ex, e)
  plt.title('Varying total recording time of EmotionalStress for each subject SpO2HR.csv file')
  plt.show()
  ...
  plt.subplot(2,2,1)
  plt.scatter(rx, r)
  plt.title('Varying total recording time of Relax for each subject in AccTempEDA.csv file')
  plt.subplot(2,2,2)
  plt.scatter(px, p)
  plt.title('Varying total recording time of PhysicalStress for each subject in AccTempEDA.csv file')
  plt.subplot(2,2,3)
  plt.scatter(cx, c)
  plt.title('Varying total recording time of CognitiveStress for each subject in AccTempEDA.csv file')
  plt.subplot(2,2,4)
  plt.scatter(ex, e)
  plt.title('Varying total recording time of EmotionalStress for each subject in AccTempEDA.csv file')
  plt.show()
...
#SANITY CHECK 
def sanity_check_1(base_dir,Subjects, SPO2HR, AccTempEDA, category = "Relax", attribute = 'Spo2', Spo2HR = True):
  """
  The base_dir: directory if the files with directory structure as follows
  -base_dir
    -subject1
  ...
  for i in Subjects:
    if Spo2HR:#path == path_2:
      assert(attribute not in ['AccZ', 'AccY', 'AccX', 'Temp', 'EDA']), f'{attribute} not the name of any column in SpO2HR.csv file. Please use either "SpO2" or "HeartRate" or set SpO2HR = False in the function calling to check AccTempEDA.csv files'
      temp_1 = i+path_2
      temp_2 = os.path.join(base_dir, i, temp_1)
      #print(temp_2)
      data2 = pd.read_csv(temp_2)
      ...
      Subject = []
      print(f'Testing {temp_2} file for the targets {attribute} for subject {count+1}')
      ...
      assert(attribute not in ['SpO2', 'HeartRate']), f'specified attribue: {attribute} not the name of a AccTempEDA.csv file column name. Please use either "AccX", "AccY", "AccZ", "Temp", "EDA" or set SpO2HR = True in fucntion calling to check SpO2HR.csv files'
      temp_1 = i+path_1
      temp_2 = os.path.join(base_dir, i, temp_1)
...
def necessary_variables(takes_nothing__HAHAHA):
  ...
  SPO2HR_target_size = {'Relax':1200, 'PhysicalStress':300, 'EmotionalStress':300, 'CognitiveStress':300} #300 set for EmotionalStress cos most
  ...
  AccTempEDA_target_size = {'Relax':9600, 'PhysicalStress':2400, 'EmotionalStress':3000, 'CognitiveStress':2400}
  ...
  attributes_dict = {'SPO2HR_attributes':SPO2HR_attributes, 'AccTempEDA_attributes': AccTempEDA_attributes}

  relax_indices = {[i*8 for i in range(1200)][i]:j for i,j in enumerate([((i+1)*8)-1 for i in range(1200)])}
  phy_emo_cog_indices = {[i*8 for i in range(300)][i]:j for i,j in enumerate([((i+1)*8)-1 for i in range(300)])}

  all_attributes = {i:j for i,j in enumerate(['SpO2', 'HeartRate', 'AccX', 'AccY', 'AccZ', 'Temp', 'EDA'])}

  return SPO2HR_target_size, AccTempEDA_target_size, SPO2HR_attributes, AccTempEDA_attributes, categories, attributes_dict, relax_indices, phy_emo_cog_indices, all_attributes
...
def resize_to_uniform_lengths(total_subject_num, categories, attributes_dict, SPO2HR_target_size, SPO2HR, AccTempEDA_target_size, AccTempEDA ):
  """
  ...
  for Class in categories: # Relax', 'CognitiveStress', 'PhysicalStress', 'EmotionalStress'
    for attributes_dict_key in attributes_dict.keys(): # SPO2HR_parameters, AccTempEDA_parameters
      ...
      for attribute in attributes_dict[attributes_dict_key]: # 'Spo2', 'HeartRate', ||| 'AccX', 'AccY', 'AccZ', 'Temp', 'EDA'
...
def sanity_check_2_and_DownSamplingAccTempEDA(total_subject_num, categories, attributes_dict, SPO2HR_target_size, SPO2HR, AccTempEDA_target_size, AccTempEDA, relax_indices, phy_emo_cog_indices ):
  for Class in categories: # Relax', 'CognitiveStress', 'PhysicalStress', 'EmotionalStress'
    for attributes_dict_key in attributes_dict.keys(): # SPO2HR_parameters, AccTempEDA_parameters
      ...
      for parameter in attributes_dict[attributes_dict_key]: # 'Spo2', 'HeartRate', ||| 'AccX', 'AccY', 'AccZ', 'Temp', 'EDA'
        if target_file == 'SPO2HR_attributes':
          ...
          #print("IN AccTempEDA NOW!!")
          target_size = AccTempEDA_target_size[Class]
          for index in range(total_subject_num): # use this line for the resizing
            ...
            if Class == 'Relax' and len(temp_list) == target_size:
              holding_list = []
              for key in relax_indices.keys(): # dict for downsapmling of the AccTempEDA values sampled at 8HZ
                temp_value = (sum(temp_list[key:relax_indices[key]]))/8
              ...
              holding_list = []
              for key in phy_emo_cog_indices.keys(): # this dict is same as the relax_indices but shorter in length. This only spans 300 values. The relax is 4 times this one.
                temp_value = (sum(temp_list[key:phy_emo_cog_indices[key]]))/8
                holding_list.append(temp_value)
              AccTempEDA[Class][parameter][index] = holding_list
            else:
              print("Passing")
...
def get_data_dict(total_subject_num, categories, attributes_dict, SPO2HR, AccTempEDA):
  ...
  for Class in categories: # Relax', 'CognitiveStress', 'PhysicalStress', 'EmotionalStress'
    for i in range(total_subject_num):
      ...
      if Class == 'Relax':
        DATA[Class][i+1] = np.array(DATA[Class][i+1]).reshape(7, 1200) # this first 'joins' all samples into a long 1-D array and then reshapes into a 2-D array
  ...
  DATA_VSTACKS['Relax'] = np.vstack([DATA['Relax'][i+1] for i in range(total_subject_num)]).reshape(total_subject_num*4, 7, 300) # all the 4 Relax stages used. So first 80 samples are relax
  DATA_VSTACKS['PhysicalStress'] = np.vstack([DATA['PhysicalStress'][i+1] for i in range(total_subject_num)]).reshape(total_subject_num,7,300)
  DATA_VSTACKS['EmotionalStress'] = np.vstack([DATA['EmotionalStress'][i+1] for i in range(total_subject_num)]).reshape(total_subject_num,7,300)
  DATA_VSTACKS['CognitiveStress'] = np.vstack([DATA['CognitiveStress'][i+1] for i in range(total_subject_num)]).reshape(total_subject_num,7,300)
  ...
  ALL_DATA = np.vstack([DATA_VSTACKS['Relax'], DATA_VSTACKS['PhysicalStress'], DATA_VSTACKS['EmotionalStress'], DATA_VSTACKS['CognitiveStress']])
